    ...
    <div class="bg-white shadow-xl rounded-2xl border border-deep-teal-200 overflow-hidden">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p class="mt-2 text-deep-teal-200 text-sm">
                        Actualice la información del aula: {{ $aula->nombre }}
                    </p>
                </div>
        ...
        <div class="p-6 sm:p-8">
            <form action="{{ route('admin.aulas.update', $aula) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="flex items-center p-4 border-2 border-deep-teal-100 rounded-xl cursor-pointer hover:bg-deep-teal-25 transition-all duration-200 has-[:checked]:border-[#3CA6A6] has-[:checked]:bg-deep-teal-50">
                            <input type="radio" name="estado" value="Disponible" {{ old('estado', $aula->estado) == 'Disponible' ? 'checked' : '' }} class="text-[#3CA6A6] focus:ring-[#3CA6A6] mr-3">
                            <div>
                        ...
                        <label class="flex items-center p-4 border-2 border-deep-teal-100 rounded-xl cursor-pointer hover:bg-deep-teal-25 transition-all duration-200 has-[:checked]:border-yellow-400 has-[:checked]:bg-yellow-50">
                            <input type="radio" name="estado" value="En Mantenimiento" {{ old('estado', $aula->estado) == 'En Mantenimiento' ? 'checked' : '' }} class="text-yellow-500 focus:ring-yellow-500 mr-3">
                            <div>
                                <div class="flex items-center">
                        ...
                        <label class="flex items-center p-4 border-2 border-deep-teal-100 rounded-xl cursor-pointer hover:bg-deep-teal-25 transition-all duration-200 has-[:checked]:border-rose-400 has-[:checked]:bg-rose-50">
                            <input type="radio" name="estado" value="No Disponible" {{ old('estado', $aula->estado) == 'No Disponible' ? 'checked' : '' }} class="text-rose-500 focus:ring-rose-500 mr-3">
                            <div>
                                <div class="flex items-center">
                ...
                <div class="bg-deep-teal-25 rounded-xl p-4 border border-deep-teal-100">
                    <h4 class="text-sm font-bold text-deep-teal-700 mb-2 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <span class="font-medium text-deep-teal-800">{{ $aula->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="text-deep-teal-600">Última actualización:</span>
                            <span class="font-medium text-deep-teal-800">{{ $aula->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
